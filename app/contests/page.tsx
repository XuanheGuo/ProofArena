...
export default async function ContestsPage() {
  ...
  const statsMap = new Map(
    contests.map((contest, index) => [contest.slug, statsPerContest[index]]),
  );
      ...
      <section className="border-b border-white/10 bg-zinc-950/90">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-6 md:py-14">
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-end">
            ...
            <div className="surface-panel grid grid-cols-3 overflow-hidden bg-zinc-950 text-center">
              <div className="border-r border-white/10 p-3">
                <strong className="font-display block text-2xl tabular-nums text-white">
                  {contests.length}
                </strong>
              ...
              <div className="border-r border-white/10 p-3">
                <strong className="font-display block text-2xl tabular-nums text-cyan-300">
                  {contests.reduce(
                    (sum, contest) => sum + contest.problems.length,
                    0,
                  )}
                </strong>
        ...
        {contests.length === 0 ? (
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            {contests.map((contest) => {
              ...
              const linkedProblems = contest.problems.filter(
                (cp) => cp.problemId || cp.draftProblemId,
              );
              ...
              return (
                <article
                  key={contest.id}
                  className={`interactive-lift surface-panel group relative flex flex-col overflow-hidden bg-zinc-950 hover:border-white/25 ${
                ...
                >
                  ...
                  <div className="flex-1 px-5 pt-5 pb-4 md:px-6 md:pt-6">
                    <div className="flex flex-wrap items-center gap-2">
                      <span
                        className={`border px-2.5 py-0.5 text-[11px] font-bold ${status.className}`}
                      >
                        {status.label}
                      </span>
                  ...
                  <div className="flex items-center justify-between gap-3 border-t border-white/[0.06] px-5 py-3 md:px-6">
                    {isDraft ? (
                      ...
                      <span className="text-xs text-zinc-600">
                        {contest.awards.length} 个奖项
                      </span>
                    )}
                    <Link
                      href={`/contests/${contest.slug}`}
                      className={`pressable pill-button inline-flex shrink-0 items-center gap-1.5 px-4 py-2 text-sm font-bold ${
                        isActive

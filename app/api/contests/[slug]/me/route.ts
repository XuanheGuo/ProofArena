...
export async function GET(_req: Request, context: RouteContext) {
  ...
  const [submissionsRes, scoresRes, profileRes, sprintAttemptsRes, awardsRes] =
    await Promise.all([
      supabase
        .from("submissions")
        .select(
          "id, problem_id, draft_problem_id, contest_problem_key, status, created_at, is_post_contest",
        )
      ...
      supabase
        ...
        .select(
          "contest_problem_id, problem_phase, raw_score, score_max, judge_note, scored_at",
        )
      ...
      supabase
        ...
        .select(
          "challenge_score, challenge_multiplier, multiplier_reason, penalty_points, penalty_reason",
        )
        .eq("contest_id", contest.id)
        .eq("user_id", user.id)
        .maybeSingle(),
      supabase
        ...
        .select(
          "contest_problem_id, unlock_at, submitted_at, elapsed_ms, is_correct, score",
        )
        .eq("contest_id", contest.id)
        .eq("user_id", user.id),
      supabase

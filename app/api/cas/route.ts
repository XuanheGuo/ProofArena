...
export async function POST(req: NextRequest) {
  ...
  const { action, ...params } = body as { action: unknown; [k: string]: unknown };
  ...
  if (action === 'equivalence') {
    ...
    if (!isNonEmptyString(expr_a) || !isNonEmptyString(expr_b)) {
      return NextResponse.json({ error: 'expr_a and expr_b must be strings' }, { status: 400 });
    }
    if (expr_a.length > MAX_EXPR_CHARS || expr_b.length > MAX_EXPR_CHARS) {
      return NextResponse.json({ error: `expr_a/expr_b must each be at most ${MAX_EXPR_CHARS} characters` }, { status: 400 });
    }
  } else {
    ...
    if (steps.length > MAX_STEPS) {
      return NextResponse.json({ error: `steps must contain at most ${MAX_STEPS} entries` }, { status: 400 });
    }
    if (!steps.every(isNonEmptyString)) {
      return NextResponse.json({ error: 'every step must be a string' }, { status: 400 });
    }
    if (steps.some((step) => step.length > MAX_STEP_CHARS)) {
      return NextResponse.json({ error: `each step must be at most ${MAX_STEP_CHARS} characters` }, { status: 400 });
    }
    const totalChars = steps.reduce((sum, step) => sum + step.length, 0);
    if (totalChars > MAX_TOTAL_STEPS_CHARS) {
      return NextResponse.json({ error: `combined steps must be at most ${MAX_TOTAL_STEPS_CHARS} characters` }, { status: 400 });
    }
  }
  ...
  try {
    ...
    if (!upstream.ok) {
      ...
      return NextResponse.json({ error: 'CAS service returned an error' }, { status: 502 });
    }
    ...
    return NextResponse.json({ error: 'CAS service unreachable' }, { status: 502 });
  }
}

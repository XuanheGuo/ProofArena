...
function Pill({
  ...
  return (
    <span
      className={`inline-flex items-center border px-2.5 py-1 text-xs font-bold ${cls}`}
    >
      {children}
    </span>
  );
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[18rem_minmax(0,1fr)] lg:items-center">
      <svg
        ...
        role="img"
        aria-label="公开主页能力雷达图"
        className="mx-auto h-72 w-full max-w-72"
      >
        {rings.map((ring) => (
          <polygon
            key={ring}
...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [solutionRes, awardRes, participantRes, scoreRes, submissionRes] =
        await Promise.all([
          supabase
            .from("solutions")
            .select(
              "id, problem_id, title, kind, contest_slug, contest_solution_type, author_role, tags, scores, challenge_target_solution_id, challenge_target_solution_title, challenge_target_solution_author, challenge_claim, created_at",
            )
          ...
          supabase
            .from("awards")
            .select(
              "id, contest_id, type, title, reason, points, created_at, contests(slug, title)",
            )
          ...
          supabase
            ...
            .select(
              "id, contest_slug, challenge_score, challenge_multiplier, multiplier_reason, penalty_points",
            )
            .eq("user_id", profileData.id),
          supabase
      ...
      setParticipantProfiles(
        (participantRes.data as ContestParticipantProfile[]) ?? [],
      );
    ...
    return (
      <div className="min-h-screen bg-zinc-950 px-4 py-16 text-center">
        <p className="text-zinc-500">
          找不到用户 <span className="text-white">@{username}</span>
        </p>
        <a
          href="/problems"
  ...
  const challenges = solutions.filter((s) =>
    Boolean(s.challenge_target_solution_id),
  );
  const contestSolutions = solutions.filter((s) => Boolean(s.contest_slug));
  const contestSlugs = new Set([
    ...contestSolutions
      .map((s) => s.contest_slug)
  ...
  const awardPoints = awards.reduce(
    (sum, award) => sum + (Number(award.points) || 0),
    0,
  );
  const officialScore = contestScores.reduce(
    (sum, score) => sum + (Number(score.raw_score) || 0),
    0,
  );
  const bestMultiplier = participantProfiles.length
    ? Math.max(
        ...participantProfiles.map((p) => Number(p.challenge_multiplier) || 1),
      )
    : 1;
  ...
  const radarMetrics: RadarMetric[] = [
    {
      ...
      label: "活跃度",
      value: clampPercent(
        (submissions.length || solutions.length) * 14 + solutions.length * 8,
      ),
      detail:
        submissions.length > 0
          ? `${submissions.length} 次投稿记录`
          : `${solutions.length} 个公开作品`,
    },
    {
      key: "participation",
      label: "参与度",
      value: clampPercent(
        contestSlugs.size * 24 +
          contestScores.length * 6 +
          contestSolutions.length * 8,
      ),
    ...
    },
    {
      ...
      label: "挑战性",
      value: clampPercent(
        challenges.length * 24 + Math.max(0, bestMultiplier - 1) * 240,
      ),
    ...
    },
    {
      ...
      detail: Number.isFinite(latestActivityDays)
        ? latestActivityDays < 1
          ? "今天有动态"
          : `${Math.floor(latestActivityDays)} 天前有动态`
        : "暂无近期动态",
    },
  ];
  const strongestDimension =
    [...radarMetrics].sort((a, b) => b.value - a.value)[0]?.label ?? null;
  const kindStats = topEntries(
    solutions.map((s) => kindLabel[s.kind] ?? s.kind),
    "暂无作品",
  );
  const styleStats = topEntries(
    solutions.map((s) =>
      s.contest_solution_type
        ? (contestSolutionTypeLabel[s.contest_solution_type] ??
          s.contest_solution_type)
        : (kindLabel[s.kind] ?? s.kind),
    ),
    "正在建立",
  );
  const tagStats = topEntries(
    solutions.flatMap((s) => s.tags ?? []).filter(Boolean),
    "暂无标签",
  ).slice(0, 6);
  const representativeSolutions = [...solutions]
        ...
        <section className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="border border-amber-400/20 bg-amber-400/[0.04] p-5">
            ...
            <div className="mt-5 grid grid-cols-3 divide-x divide-white/[0.08] border border-white/10 bg-black/20 text-center">
              ...
              <div className="p-3">
                <strong className="block font-display text-2xl text-amber-200">
                  {awardPoints}
                </strong>
                <span className="text-[11px] text-zinc-500">奖励分</span>
              </div>
              <div className="p-3">
                <strong className="block font-display text-2xl text-cyan-200">
          ...
          <div className="border border-white/10 bg-white/[0.02] p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {kindStats.slice(0, 4).map(([label, count]) => (
                <Pill key={label} tone={count > 0 ? "violet" : "zinc"}>
                  {label} · {count}
                </Pill>
              ))}
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {awards.slice(0, 6).map((award) => {
                ...
                return (
                  <article
                    key={award.id}
                    className="border border-amber-400/20 bg-amber-400/[0.04] p-4"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <Pill tone="amber">
                          {awardTypeLabel[award.type] ?? award.type}
                        </Pill>
                      ...
                      <strong className="font-display text-xl text-amber-200">
                        +{award.points}
                      </strong>
                    </div>
                    {award.reason && (
          ...
          <div className="flex items-center justify-between">
            ...
            {strongestDimension && (
              <span className="text-xs text-zinc-500">
                代表优势：
                <span className="text-cyan-300">{strongestDimension}</span>
              </span>
            )}
          </div>
                  ...
                  >
                    ...
                    {s.challenge_target_solution_id && (
                      <p className="mt-3 inline-flex items-center gap-1.5 border border-amber-400/20 bg-amber-400/[0.05] px-2.5 py-1 text-xs text-amber-200">
                        ...
                        挑战{" "}
                        <MathBlock>
                          {s.challenge_target_solution_title ??
                            s.challenge_target_solution_id}
                        </MathBlock>
                      </p>
                    )}

...
} from "@/components/EditSubmissionForm";
...
const SUBMISSIONS_SELECT =
  "id, title, submission_type, problem_id, problem_source, kind, status, failure_reason, created_at, updated_at, moderator_notes, content, contest_id, contest_slug, attachment_urls, challenge_target_solution_id, challenge_claim, challenge_advantages, challenge_risk";
...
function SubmissionProgress({ submission }: { submission: Submission }) {
        ...
        <div className="flex items-center gap-0">
          <div className="flex items-center">
            <div className="flex flex-col items-center">
              <div className="flex size-7 items-center justify-center border-2 border-cyan-400 bg-cyan-400 text-xs font-bold text-zinc-950">
                <CheckCircle2 className="size-4" />
              </div>
              <span className="mt-1.5 text-[10px] font-medium text-white whitespace-nowrap">
                已提交
              </span>
            </div>
          ...
          <div className="flex items-center">
            <div className="flex flex-col items-center">
              <div className="flex size-7 items-center justify-center border-2 border-current text-amber-400">
                <AlertCircle className="size-4" />
              </div>
              <span className="mt-1.5 text-[10px] font-medium text-amber-400 whitespace-nowrap">
                预筛未通过
              </span>
            </div>
          </div>
        </div>
        <div className="mt-4 border border-amber-400/30 bg-amber-400/[0.06] px-4 py-3">
          <p className="text-xs font-bold text-amber-300">未进入人工审核</p>
          <p className="mt-1 text-sm text-zinc-300">
            {getSubmissionFailureReasonLabel(failure_reason)}
            ，请修改后重新提交。
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-0">
        {steps.map((step, i) => (
          <div key={step.key} className="flex items-center">
            <div className="flex flex-col items-center">
              <div
                className={`flex size-7 items-center justify-center border-2 text-xs font-bold transition
 ${
 ...
 ${step.active ? "text-amber-400" : step.done ? (i === steps.length - 1 ? resultColor : "text-white") : "text-zinc-600"}`}
            ...
            {i < steps.length - 1 && (
              <div
                className={`mb-5 h-px w-12 sm:w-20 ${step.done ? "bg-cyan-400" : "bg-white/10"}`}
              />
            )}
          </div>
        ))}
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    client.auth.getUser().then(async ({ data }) => {
      ...
      const { data: solutions } = await client
        .from("solutions")
        .select(
          "id, problem_id, title, kind, author_role, scores, challenge_target_solution_id, challenge_target_solution_title, challenge_target_solution_author, challenge_claim, challenge_advantages, challenge_risk, created_at, source_submission_id",
        )
      ...
      <div className="min-h-screen bg-zinc-950 px-4 py-16">
        <div className="mx-auto max-w-2xl border border-amber-400/25 bg-amber-400/[0.06] p-6 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-amber-100">
            个人主页、投稿记录和公开作品集需要
            Supabase。当前仍可浏览静态题库和解法。
          </p>
          <a
            href="/problems"
            className="mt-5 inline-block text-sm font-bold text-cyan-300 hover:text-cyan-200"
          >
            返回题库
          </a>
        </div>
  ...
  const getSubmissionMeta = (sub: Submission) => {
    ...
    const target =
      sub.submission_type === "problem"
        ? sub.problem_source
        : sub.problem_source || sub.problem_id;
    return [type, target, kindLabel[sub.kind] ?? sub.kind]
      .filter(Boolean)
      .join(" · ");
  };

  const solutionSubmissions = submissions.filter(
    (sub) => sub.submission_type !== "problem",
  );
  ...
  const challengeSubmissions = solutionSubmissions.filter((sub) =>
    Boolean(
      sub.challenge_target_solution_id ||
      sub.content?.json?.solution?.challenge?.targetSolutionId,
    ),
  );
  const publishedChallenges = publishedSolutions.filter((solution) =>
    Boolean(solution.challenge_target_solution_id),
  );
  const averageScore = publishedSolutions.length
    ? publishedSolutions
        .map((solution) => {
          const values = Object.values(solution.scores ?? {}).filter(
  ...
  return (
    <div className="min-h-screen bg-zinc-950 px-4 py-16">
      <div className="mx-auto max-w-5xl space-y-8">
        ...
        <div className="space-y-3 border border-white/10 bg-white/[0.02] p-6">
        ...
        <ProfileSettingsPanel
          ...
          onUsernameUpdated={(next) =>
            setAppProfile((current) =>
              current ? { ...current, username: next } : current,
            )
          }
        />
        ...
        <section className="grid gap-3 md:grid-cols-4">
          {[
            ...
            [
              "挑战记录",
              challengeSubmissions.length + publishedChallenges.length,
              Swords,
              "text-amber-300",
            ],
            [
        ...
        <section className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
          ...
          <div className="border border-amber-400/20 bg-amber-400/[0.04] p-5">
            ...
            <a
              href="/submit"
              className="mt-4 inline-flex h-9 items-center justify-center bg-amber-300 px-4 text-xs font-bold text-zinc-950 transition hover:bg-amber-200"
            >
              发起解法挑战
            </a>
          </div>
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              {representativeSolutions.map((solution) => {
                const scores = Object.values(solution.scores ?? {}).filter(
                  (value): value is number => typeof value === "number",
                );
                ...
                return (
                  <a
                    key={solution.id}
                    href={`/problems/${solution.problem_id}#${solution.id}`}
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <span className="border border-white/10 px-2 py-1 text-xs text-zinc-400">
                        {kindLabel[solution.kind] ?? solution.kind}
                      </span>
                    ...
                    {solution.challenge_target_solution_id && (
                      <p className="mt-4 border border-amber-400/20 bg-amber-400/[0.05] px-3 py-2 text-xs leading-5 text-amber-100">
                        挑战{" "}
                        <MathBlock>
                          {solution.challenge_target_solution_title ??
                            solution.challenge_target_solution_id}
                        </MathBlock>
                      </p>
                    )}
        ...
        <div className="space-y-4">
          ...
          {submissions.length === 0 ? (
            <div className="border border-white/10 bg-white/[0.02] p-8 text-center">
              ...
              <a
                href="/submit"
                className="mt-4 inline-block bg-cyan-400 px-4 py-2 text-sm font-bold text-zinc-950 transition hover:bg-cyan-300"
              >
                提交第一份解法
              </a>
            </div>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {(sub.challenge_target_solution_id ||
                        sub.content?.json?.solution?.challenge
                          ?.targetSolutionId) && (
                        <p className="mt-2 inline-flex items-center gap-1.5 border border-amber-400/25 bg-amber-400/[0.06] px-2.5 py-1 text-xs text-amber-200">

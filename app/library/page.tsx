...
function insightsForCategory(category: string) {
  ...
  return insightNodes.filter((node) =>
    node.relatedKnowledgeIds.some((id) => knowledgeIds.has(id)),
  );
}
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12">
        <div className="grid gap-5">
          {categories.map((category) => {
            const nodes = knowledgeNodes.filter(
                    ...
                    <div className="grid gap-3">
                      {nodes.map((node) => (
                        <Link
                          ...
                          className="group border border-white/10 bg-black/20 p-4 transition hover:border-cyan-400/40"
                        >
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            {node.aliases.slice(0, 3).map((alias) => (
                              <span
                                key={alias}
                                className="border border-cyan-400/15 px-2 py-1 text-[11px] text-zinc-500"
                              >
                                {alias}
                              </span>
                            ))}
                      ...
                      {insights.length > 0 ? (
                        insights.map((node) => (
                          <Link
                            key={node.id}
                            href={`/library/${node.id}`}
                            className="group border border-white/10 bg-black/20 p-4 transition hover:border-amber-400/40"
                          >
                            <div className="flex items-start justify-between gap-3">
                              <h3 className="font-bold text-white">
                                {node.title}
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                              {node.appliesTo.slice(0, 3).map((item) => (
                                <span
                                  key={item}
                                  className="inline-flex items-center gap-1 border border-amber-400/15 px-2 py-1 text-[11px] text-zinc-500"
                                >

...
export default async function LibraryDetailPage({
  ...
  const category =
    knowledge?.category ??
    getKnowledgeNode(insight?.relatedKnowledgeIds[0] ?? "")?.category ??
    "思路触发";
  const summary = knowledge?.summary ?? insight?.idea ?? "";
  const trigger =
    insight?.trigger ??
    knowledge?.examples[0] ??
    "看到相关标签、题型结构或目标量时，可以回到这个节点检查是否适用。";
  const relatedKnowledgeIds =
    knowledge?.relatedIds ?? insight?.relatedKnowledgeIds ?? [];
  const relatedKnowledge = relatedKnowledgeIds
  ...
  const relatedProblems = problems.filter((problem) => {
    const problemHit =
      problem.knowledgeIds?.includes(id) || problem.insightIds?.includes(id);
    const solutionHit = problem.solutions.some(
      (solution) =>
        solution.knowledgeIds?.includes(id) ||
        solution.insightIds?.includes(id),
    );
    return (
      problemHit ||
      solutionHit ||
      insight?.relatedProblemIds.includes(problem.id)
    );
  });
        ...
        <div className="mx-auto max-w-5xl px-5 py-10 sm:px-6 md:py-14 lg:px-8">
          ...
          <div className="mt-8">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <span className="bg-cyan-400 px-2 py-1 text-xs font-bold text-zinc-950">
                {category}
              </span>
              <span className="border border-white/10 px-2 py-1 text-xs text-zinc-400">
                {isKnowledge ? "知识点" : "思路触发"}
              </span>
              {insight && (
                <span className="border border-amber-400/25 px-2 py-1 text-xs text-amber-300">
                  {insight.difficulty}
                </span>
              )}
            </div>
                ...
                <div className="border border-white/10 bg-black/20 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {knowledge.prerequisites.map((item) => (
                      <span
                        key={item}
                        className="border border-white/10 px-2.5 py-1.5 text-xs text-zinc-400"
                      >
                        {item}
                      </span>
                    ))}
                ...
                {relatedKnowledge.length > 0 ? (
                  relatedKnowledge.map((node) => (
                    <Link
                      ...
                      className="group border border-white/10 bg-black/20 p-3 transition hover:border-cyan-400/40"
                    >
            ...
            <section className="border border-white/10 bg-zinc-950 p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {exampleTags.map((tag) => (
                  <span
                    key={tag}
                    className="border border-amber-400/20 bg-amber-400/5 px-2.5 py-1.5 text-xs text-zinc-300"
                  >
                    {tag}
                  </span>
                ))}
          ...
          <div className="grid gap-px bg-white/10 md:grid-cols-2">
            {relatedProblems.map((problem) => (
              <Link
                ...
                className="group bg-zinc-950 p-5 transition hover:bg-white/[0.03]"
              >
            ...
            {relatedSolutions.map(({ problem, solution }) => (
              <Link
                ...
                href={`/problems/${problem.id}#${solution.id}`}
                className="group bg-zinc-950 p-5 transition hover:bg-white/[0.03]"
              >

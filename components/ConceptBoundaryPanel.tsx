...
function linkedConceptHref(link: ConceptLink) {
  return link.conceptId && getKnowledgeNode(link.conceptId)
    ? `/library/${link.conceptId}`
    : undefined;
}
...
export function ConceptBoundaryPanel(props: ConceptBoundaryPanelProps) {
  ...
  return (
    <section
      id={id}
      className={`scroll-mt-32 border border-amber-400/25 bg-zinc-950 ${className}`}
    >
      ...
      <div
        className={`grid gap-px bg-white/10 ${compact ? "" : "xl:grid-cols-2"}`}
      >
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2">
              {conceptLinks.map((link) => {
                ...
                return href ? (
                  <Link
                    key={`${link.label}-${link.relation}`}
                    href={href}
                    className="group border border-cyan-400/20 bg-cyan-400/[0.04] p-4 transition hover:border-cyan-400/45"
                  >
                    {body}
                  </Link>
                ) : (
                  <div
                    key={`${link.label}-${link.relation}`}
                    className="border border-cyan-400/20 bg-cyan-400/[0.04] p-4"
                  >
                    {body}
                  </div>
                );
          ...
          >
            <div className="space-y-3">
              {conceptContrasts.map((contrast) => (
                <div
                  key={`${contrast.conceptA}-${contrast.conceptB}`}
                ...
                >
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <BoundaryCallout
                      label="核心区别"
                      tone="amber"
                      text={contrast.keyDifference}
                    />
                    <BoundaryCallout
                      label="常见误用"
                      tone="red"
                      text={contrast.commonMistake}
                    />
                  </div>
                      ...
                      {contrast.exampleProblemIds.map((problemId) => {
                        ...
                        return (
                          <Link
                            ...
                            className="border border-white/10 px-2.5 py-1.5 text-xs text-zinc-400 transition hover:border-amber-400/40 hover:text-amber-200"
                          >
          ...
          >
            <div className="space-y-3">
              {boundaryNotes.map((note) => (
                <div
                  key={note.title}
                  className="border-l-2 border-emerald-400 bg-emerald-400/[0.04] p-4"
                >
                  <h3 className="text-sm font-bold text-white">
                    <MathBlock>{note.title}</MathBlock>
                  </h3>
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2">
              {contrastProblems.map((item) => {
                ...
                return (
                  <Link
                    key={`${item.problemId}-${item.focus}`}
                    href={`/problems/${item.problemId}`}
                    className="group border border-white/10 bg-black/20 p-4 transition hover:border-cyan-400/35"
                  >
        ...
        {whyNotMethods.length > 0 && (
          <ConceptSection
            title="为什么不是这个方法？"
            icon={<RouteOff className="size-4" />}
            tone="red"
            defaultOpen={false}
            wide
          >
            <div className="grid gap-3 md:grid-cols-2">
              {whyNotMethods.map((method) => (
                <div
                  ...
                  className="border border-red-400/20 bg-red-500/[0.04] p-4"
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {method.relatedConcepts.map((concept) => (
                      <span
                        key={concept}
                        className="border border-white/10 px-2 py-1 text-[11px] text-zinc-500"
                      >
                        {concept}
                      </span>
                    ))}
...
function ConceptSection({
  ...
  return (
    <details
      open={defaultOpen}
      className={`group bg-zinc-950 ${wide ? "xl:col-span-2" : ""}`}
    >
      <summary className="flex min-h-14 list-none items-center justify-between gap-3 border-b border-white/10 px-5 py-4 marker:hidden">
        <span
          className={`flex items-center gap-2 text-xs font-bold ${toneClass}`}
        >
          {icon}
          {title}
        </span>

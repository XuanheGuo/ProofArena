...
> = {
  ...
  precheck_failed: {
    label: "预筛未通过",
    className: "border-orange-500/40 bg-orange-500/[0.07] text-orange-300",
    icon: AlertCircle,
  },
};
...
function matchesContestProblem(sub: MySubmission, cp: ContestProblem) {
  ...
  if (sub.draftProblemId && sub.draftProblemId === cp.draftProblemId)
    return true;
  return false;
}
...
function Stat({
  ...
  return (
    <div className="surface-panel-subtle px-3 py-2 text-center">
      <strong
        className={`block font-mono text-base font-bold tabular-nums ${accent ? "text-amber-300" : "text-white"}`}
      >
        {value}
      </strong>
...
export function ContestMyPanel({
  ...
  const [user, setUser] = useState<{ id: string } | null | undefined>(
    undefined,
  );
  ...
  if (user === null) {
    return (
      <section className="surface-panel p-5">
        ...
        <div className="surface-panel-subtle mt-4 flex items-center gap-3 border-amber-400/20 bg-amber-400/[0.04] px-4 py-3">
  ...
  const challengeMultiplier =
    summary?.participantProfile?.challengeMultiplier ?? 1;
  const penaltyPoints = summary?.participantProfile?.penaltyPoints ?? 0;
  const awardPoints = summary?.awardPoints ?? 0;

  const dailyRawScore = scores
    .filter((s) => s.problemPhase === "daily")
    .reduce((sum, s) => sum + s.rawScore, 0);
  const majorScore = scores
    .filter((s) => s.problemPhase === "major")
    .reduce((sum, s) => sum + s.rawScore, 0);
  const sprintScore = sprintAttempts.reduce((sum, a) => sum + a.score, 0);
  const dailyFinalScore = dailyRawScore * challengeMultiplier;
  const totalScore =
    dailyFinalScore + sprintScore + majorScore + awardPoints - penaltyPoints;

  const hasAnyScoring =
    scores.length > 0 ||
    sprintAttempts.length > 0 ||
    summary?.participantProfile;
    ...
    <section className="surface-panel p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="text-xs text-zinc-500">
          {totalSubs > 0 &&
            `${totalSubs} 份投稿${approvedSubs > 0 ? `，${approvedSubs} 份已通过` : ""}`}
        </div>
      </div>
        ...
        <>
          {hasAnyScoring && (
            <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-6">
              ...
              <Stat
                label="挑战倍率"
                value={`${challengeMultiplier.toFixed(2)}x`}
              />
              <Stat
                label="普通题结算分"
                value={dailyFinalScore.toFixed(1)}
                accent
              />
          ...
          <div className="mt-4 space-y-2">
            {contest.problems.map((cp) => {
              ...
              if (cp.problemPhase === "sprint") {
                const attempt = sprintAttempts.find(
                  (a) => a.contestProblemId === cp.id,
                );
                ...
                return (
                  <div
                    key={cp.id}
                    className="surface-panel-subtle grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3"
                  >
                      ...
                      {attempt?.submittedAt && (
                        <p className="mt-0.5 text-[11px] text-zinc-600">
                          ...
                          {attempt.elapsedMs != null
                            ...
                            : "—"}{" "}
                          · 提交于 {formatContestDateTime(attempt.submittedAt)}
                        </p>
                      )}
                    </div>
                    <span
                      className={`shrink-0 inline-flex items-center gap-1 border px-2 py-0.5 text-[11px] font-bold ${sprintBadge.className}`}
                    >
              ...
              const mySubs = submissions.filter((s) =>
                matchesContestProblem(s, cp),
              );
              const officialSubs = mySubs.filter((s) => !s.isPostContest);
              const latestSub = mySubs[0];
              const bestStatus =
                officialSubs.find((s) => s.status === "approved")?.status ??
                officialSubs.find((s) => s.status === "needs_revision")
                  ?.status ??
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    {latestSub && (
                      <p className="mt-0.5 text-[11px] text-zinc-600">
                        {mySubs.length} 次投稿 · 最近：
                        {formatContestDateTime(latestSub.createdAt)}
                      </p>
                    )}
                  </div>

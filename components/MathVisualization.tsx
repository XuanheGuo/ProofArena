...
type VisualizationKind =
  "tianjin-tangent-ellipse" | "trajectory-classifier" | "derivative-proof";
...
const visualizationSpecs: Record<string, VisualizationSpec> = {
  "tj-2026-18": {
    ...
    title: "切线与斜率比实验台",
    description:
      "拖动截距滑块，观察斜率为 -√3 的直线何时与圆相切、何时切出椭圆上的 P,Q，并比较 AP 与 AQ 的斜率。",
    insight:
      "这题的图像核心不是椭圆焦半径，而是“圆的切线确定直线，直线截椭圆确定两个斜率”。有效切线落在截距 2√3。",
  ...
  "ng2-2026-18": {
    ...
    title: "动点轨迹分类实验台",
    description:
      "拖动 θ 让 A 在椭圆上运动，拖动 t₀ 改变点 G。绿色点 P 的轨迹会随 t₀ 从双曲线型过渡到抛物线型，再到椭圆型。",
    insight:
      "轨迹方程里的 x² 系数 1/2-1/t₀² 决定曲线类型；临界值 t₀=√2 时，二次项消失，图像变成抛物线型。",
  ...
  "tj-2026-20": {
    ...
    title: "差函数与临界指数实验台",
    description:
      "拖动 x₀ 查看差函数 g(x)=f(x)-1-x/3 的高度；再拖动 n，观察乘积归一化 Pₙ/(n+1)^(1/3) 是否低于 1。",
    insight:
      "切线图只能看到下界；差函数图说明为什么下界成立，离散乘积条则说明为什么 1/3 是自然出现的临界指数。",
...
export function MathVisualization({ problemId }: { problemId: string }) {
  ...
  useEffect(() => {
    ...
    async function initialize() {
      ...
      try {
        ...
        if (spec.kind === "tianjin-tangent-ellipse")
          drawTianjinTangentEllipse(board);
        if (spec.kind === "trajectory-classifier")
          drawTrajectoryClassifier(board);
  ...
  return (
    <section
      id="visualization"
      className="mt-5 scroll-mt-32 border border-cyan-400/25 bg-zinc-950"
    >
...
async function withTimeout<T>(promise: Promise<T>, timeoutMs: number) {
  ...
  const timeout = new Promise<never>((_, reject) => {
    timeoutId = setTimeout(
      () => reject(new Error("Timed out loading JSXGraph")),
      timeoutMs,
    );
  });
  try {
...
function drawTianjinTangentEllipse(board: JXG.Board) {
  ...
  const movingIntercept = board.create(
    "slider",
    [
      [-2.7, 3.75],
      [1.5, 3.75],
      [-tangentIntercept, tangentIntercept, tangentIntercept],
    ],
    {
      name: "s",
      snapWidth: 0.02,
  ...
  board.create("point", [0, b], {
    ...basePointStyle("A", colors.green),
    fixed: true,
  });
  board.create("point", [2, 0], {
    ...basePointStyle("P", colors.amber),
    fixed: true,
  });
  board.create("point", [6 / 5, (4 * Math.sqrt(3)) / 5], {
    ...basePointStyle("Q", colors.amber),
    fixed: true,
  });

  board.create(
    "functiongraph",
    [(x: number) => -Math.sqrt(3) * x + movingIntercept.Value(), -3.2, 4.2],
    {
  ...
  board.create(
    "functiongraph",
    [(x: number) => -Math.sqrt(3) * x + tangentIntercept, -3.2, 4.2],
    {
  ...
  const foot = board.create(
    ...
    { ...basePointStyle("H", colors.violet), fixed: true },
  ) as JXG.Point;
  board.create("segment", [[0, 0], foot], {
    strokeColor: colors.violet,
    strokeWidth: 2,
    dash: 2,
  });
  ...
  board.create(
    ...
    [
      ...
      -1.75,
      () =>
        `圆心到动直线距离 = ${Math.abs(movingIntercept.Value() / 2).toFixed(2)}；半径 = √3 ≈ ${b.toFixed(2)}`,
    ],
    textStyle(colors.violet),
  );
  board.create(
    "text",
    [-2.8, -2.12, "有效切线：y = -√3x + 2√3"],
    textStyle(colors.green),
  );
  board.create(
    "text",
    [0.8, -2.12, "k_AP / k_AQ = 3"],
    textStyle(colors.amber),
  );
}

function drawTrajectoryClassifier(board: JXG.Board) {
  const root2 = Math.sqrt(2);
  const theta = board.create(
    "slider",
    [
      [-5.2, 4.15],
      [-1.1, 4.15],
      [0.08, 1.12, 2 * Math.PI - 0.08],
    ],
    {
      name: "θ",
      snapWidth: 0.01,
  ...
  const safePx = (u: number) =>
    Math.abs(denom(u)) < 0.05 ? Number.NaN : px(u);
  const safePy = (u: number) =>
    Math.abs(denom(u)) < 0.05 ? Number.NaN : py(u);

  drawEllipse(board, root2, 1, colors.cyan);
  const a = board.create(
    "point",
    [() => root2 * Math.cos(theta.Value()), () => Math.sin(theta.Value())],
    basePointStyle("A", colors.cyan),
  ) as JXG.Point;
  const b = board.create(
    "point",
    [0, () => Math.sin(theta.Value())],
    basePointStyle("B", colors.zinc),
  ) as JXG.Point;
  const g = board.create(
    "point",
    [() => t0(), 0],
    basePointStyle("G", colors.amber),
  ) as JXG.Point;
  const p = board.create(
    "point",
    [() => safePx(theta.Value()), () => safePy(theta.Value())],
    basePointStyle("P", colors.green),
  ) as JXG.Point;

  board.create("line", [[0, 0], a], {
    straightFirst: false,
    straightLast: true,
    strokeColor: colors.cyan,
    strokeWidth: 2,
  ...
  board.create(
    ...
    [
      -5.4,
      -3.65,
      () => `1/2 - 1/t₀² = ${(0.5 - 1 / (t0() * t0())).toFixed(3)}`,
    ],
    textStyle(colors.amber),
  );
  board.create(
    "text",
    [-5.4, -4.05, () => `当前类型：${classifyTrajectory(t0())}`],
    textStyle(colors.green),
  );
  board.create(
    "text",
    [0.85, -4.05, `临界：t₀ = √2 ≈ ${root2.toFixed(3)}`],
...
function drawDerivativeProof(board: JXG.Board) {
  ...
  board.create("functiongraph", [g, -1 / 3, 2.15], {
    strokeColor: colors.green,
    strokeWidth: 3,
    name: "g(x)",
    highlight: false,
  });
  board.create("functiongraph", [gp, -1 / 3, 2.15], {
    strokeColor: colors.violet,
    strokeWidth: 2,
    dash: 2,
    name: "g'(x)",
  ...
  const point = board.create(
    "point",
    [() => xSlider.Value(), () => g(xSlider.Value())],
    basePointStyle("g(x₀)", colors.green),
  ) as JXG.Point;
  const shadow = board.create("point", [() => xSlider.Value(), 0], {
    ...basePointStyle("", colors.zinc),
    visible: false,
  }) as JXG.Point;
  board.create("segment", [point, shadow], {
    strokeColor: colors.amber,
    strokeWidth: 4,
  });
  ...
  board.create(
    ...
    [
      ...
      -0.18,
      () =>
        `g(x₀) = ${g(xSlider.Value()).toFixed(4)}，g'(x₀) = ${gp(xSlider.Value()).toFixed(4)}`,
    ],
    textStyle(colors.green),
  );
  board.create(
    ...
    [
      ...
      -0.29,
      () =>
        `Pₙ/(n+1)^(1/3) = ${normalizedProduct(Math.round(nSlider.Value())).toFixed(4)} ；n = ${Math.round(nSlider.Value())}`,
    ],
    textStyle(colors.amber),
  );
  board.create(
    "text",
    [1.02, 0.28, "绿色：差函数 g(x)"],
    textStyle(colors.green),
  );
  board.create(
    "text",

...
import {
  weekly01DraftProblems,
  weekly01SprintAnswerKeys,
} from "@/data/weekly01-drafts";
import {
  accessModeMeta,
  contestAwardMeta,
  contestProblemPhaseMeta,
  contestRegistrationStatusMeta,
  contestSolutionTypeMeta,
  contestStatusMeta,
} from "@/lib/contest-meta";
...
import {
  inviteContestParticipant,
  reviewContestRegistration,
} from "@/lib/contest-registration-actions";
...
function draftSourceLabel(
  draft: Pick<DbProblemDraft, "year" | "region" | "paper" | "number">,
) {
  return [
    draft.region,
    draft.paper,
...
export function AdminContestsView({
  problems,
  initialDraftProblems = [],
}: {
  problems: ProblemOption[];
  initialDraftProblems?: DbProblemDraft[];
}) {
  ...
  const [problemSourceMode, setProblemSourceMode] = useState<
    "public" | "draft"
  >("public");
  const [awardForm, setAwardForm] = useState(emptyAward);
  const [awardParticipants, setAwardParticipants] = useState<
    AwardParticipantOption[]
  >([]);
  ...
  const [draftProblems, setDraftProblems] =
    useState<DbProblemDraft[]>(initialDraftProblems);
  ...
  const awardSolutionMap = useMemo(
    () =>
      new Map(
        awardSolutions.map((solution) => [
          `${solution.source}:${solution.id}`,
          solution,
        ]),
      ),
    [awardSolutions],
    ...
    async function loadAwardOptions() {
      ...
      const [submissionsRes, solutionsRes] = await Promise.all([
        supabase
          .from("submissions")
          .select(
            "id, title, user_id, problem_id, draft_problem_id, contest_problem_key, contest_solution_type, status, created_at",
          )
        ...
        supabase
          .from("solutions")
          .select(
            "id, title, author, author_id, problem_id, source_submission_id, contest_problem_key, contest_solution_type",
          )
          .eq("contest_slug", contestForOptions.slug)
      ...
      const publishedSolutions: AwardSolutionOption[] = (
        solutionsRes.data ?? []
      ).map((solution) => ({
        ...
        draftProblemId: null,
        contestProblemKey:
          (solution.contest_problem_key as string | null) ?? null,
        contestSolutionType:
          (solution.contest_solution_type as string | null) ?? null,
      }));
      const approvedContestSubmissions: AwardSolutionOption[] = (
        submissionsRes.data ?? []
      )
        .filter((submission) => submission.status === "approved")
        .filter(
          (submission) =>
            !publishedSourceSubmissionIds.has(submission.id as string),
        )
        .map((submission) => {
          ...
          return {
            ...
            author: authorId
              ? (profileNames.get(authorId) ?? `用户 ${authorId.slice(0, 8)}`)
              : "匿名",
            authorId,
            problemId: (submission.problem_id as string | null) ?? null,
            draftProblemId:
              (submission.draft_problem_id as string | null) ?? null,
            contestProblemKey:
              (submission.contest_problem_key as string | null) ?? null,
            status: submission.status as string,
            contestSolutionType:
              (submission.contest_solution_type as string | null) ?? null,
          };
        });
  ...
  async function loadRegistrations(contestId: string) {
    ...
    setRegistrations(
      (data ?? []).map((row) => ({
        ...
        label:
          profileNames.get(row.user_id as string) ??
          `用户 ${(row.user_id as string).slice(0, 8)}`,
  ...
  async function handleReviewRegistration(
    ...
    setError("");
    setMessage("");
    const result = await reviewContestRegistration({
      registrationId,
      nextStatus,
    });
  ...
  async function handleInvite(userId: string) {
    ...
    setError("");
    setMessage("");
    const result = await inviteContestParticipant({
  ...
  async function loadContests() {
    ...
    if (loadError) {
      setError(
        loadError.message ||
          "加载比赛失败。请确认已执行 004_contest_arena_mvp.sql。",
      );
      return;
    }
    ...
    setContests(rows);
    if (!selectedId) {
      const requestedContest = contestParam
        ? rows.find((contest) => contest.slug === contestParam)
        : null;
      if (requestedContest ?? rows[0])
        setSelectedId((requestedContest ?? rows[0]).id);
    }
  }
  ...
  async function syncSeedContest(seedSlug: string) {
    ...
    const { data: existingProblems, error: existingProblemsError } =
      await supabase
    ...
    if (existingProblemsError) {
      setSaving(false);
      setError(
        existingProblemsError.message || "读取现有赛题失败，未同步任何赛题。",
      );
      return;
    }
    ...
    const existingIdByKey = new Map(
      (existingProblems ?? []).map((row) => [rowKey(row), row.id as string]),
    );
    ...
    const sprintAnswerKeyByDraftId = new Map(
      weekly01SprintAnswerKeys.map((answerKey) => [
        answerKey.draftProblemId,
        answerKey,
      ]),
    );
    ...
    for (const contestProblem of seed.problems) {
      ...
      const matchedId =
        (contestProblem.draftProblemId
          ? existingIdByDraftId.get(contestProblem.draftProblemId)
          : undefined) ??
        existingIdByKey.get(
          rowKey({
            day_index: contestProblem.dayIndex,
      ...
      const sprintAnswerKey = contestProblem.draftProblemId
        ? sprintAnswerKeyByDraftId.get(contestProblem.draftProblemId)
        : undefined;
      if (contestProblemId && sprintAnswerKey) {
        ...
        if (answerKeyError) {
          setSaving(false);
          setError(
            `同步「${contestProblem.title}」答案 key 失败：${answerKeyError.message}`,
          );
          await loadContests();
          return;
        }
    ...
    setSaving(false);
    setMessage(
      `已同步「${seed.title}」：新增 ${insertedCount} 个赛题，更新 ${updatedCount} 个赛题，写入 ${answerKeyCount} 个计时题答案 key，清理 ${staleEmptyDeletedCount} 个旧空槽位。`,
    );
    await Promise.all([loadContests(), loadDraftProblems()]);
  ...
  async function saveContest() {
    ...
    const patch = {
      ...
      discussion_start_at: contestForm.discussionStartAt
        ? fromInputDate(contestForm.discussionStartAt)
        : null,
      discussion_end_at: contestForm.discussionEndAt
        ? fromInputDate(contestForm.discussionEndAt)
        : null,
    };
  ...
  async function addContestProblem() {
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        time_limit_seconds: problemForm.timeLimitSeconds
          ? Number(problemForm.timeLimitSeconds)
          : null,
  ...
  async function updateContestProblem(
    problem: DbContestProblem,
    patch: Partial<DbContestProblem>,
  ) {
    setSaving(true);
    setError("");
    const { error: updateError } = await supabase
  ...
  function updateAwardSolution(value: string) {
    ...
    const solution = awardSolutions.find(
    ...
    return (
      <div className="mt-10 space-y-3">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-20 animate-pulse border border-white/10 bg-white/[0.03]"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="mt-8 grid gap-6 lg:grid-cols-[17rem_minmax(0,1fr)]">
      <aside className="space-y-3 lg:sticky lg:top-24 lg:self-start">
        ...
        <section className="border border-white/10 bg-zinc-950 p-4">
          ...
          <p className="mt-2 text-[11px] leading-4 text-zinc-600">
            Weekly 01
            是空题目槽位模板，同步后需要在下方「赛题安排」逐个绑定真实题目、配置计时题答案
            key。
          </p>
        </section>
          ...
          <div className="divide-y divide-white/[0.07]">
            {contests.map((contest) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`mt-1.5 inline-flex items-center border px-2 py-0.5 text-[11px] font-bold ${status.className}`}
                  >
                    {status.label}
                  </span>
                </button>
            ...
            {contests.length === 0 && (
              <p className="px-4 py-6 text-sm text-zinc-500">
                还没有比赛。点击上方"同步 Invitational 01"或"创建/同步 Weekly
                01"开始。
              </p>
            )}
          </div>
      ...
      <div className="min-w-0 space-y-5">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              {selectedContest
                ? `比赛 ID: ${selectedContest.id.slice(0, 8)}…`
                : "填写信息后保存"}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <Link
              ...
              className="inline-flex h-7 items-center border border-white/15 px-3 text-zinc-300 transition hover:border-white/30 hover:text-white"
            >
              投稿审核
            </Link>
            {selectedContest && (
              <>
                <Link
                  href={`/contests/${selectedContest.slug}`}
                  className="inline-flex h-7 items-center border border-white/15 px-3 text-zinc-300 transition hover:border-white/30 hover:text-white"
                  target="_blank"
                >
                  前台预览 ↗
        ...
        {error && (
          <div className="border border-red-500/40 bg-red-500/[0.08] px-4 py-2.5 text-sm text-red-300">
            {error}
          </div>
        )}
        ...
        {selectedContest &&
          (() => {
            ...
            const overdueStart =
              selectedContest.status === "draft" && now >= startAt;
            const overdueEnd =
              selectedContest.status === "active" && now >= endAt;
            return overdueStart || overdueEnd ? (
              <div className="flex items-start gap-3 border border-red-500/50 bg-red-500/[0.08] px-4 py-3">
                ...
                <p className="text-sm leading-6 text-red-200">
                  {overdueStart && (
                    <>
                      <strong className="text-red-300">开赛时间已过</strong>
                      ，但比赛状态仍是「未开始」。请点击下方「开始比赛」按钮将状态切换为进行中。
                    </>
                  )}
                  {overdueEnd && (
                    <>
                      <strong className="text-red-300">截止时间已过</strong>
                      ，但比赛状态仍是「进行中」。请点击下方「进入评审」按钮关闭投稿通道。
                    </>
                  )}
                </p>
              </div>
        ...
        {selectedContest && (
          <section className="border border-white/10 bg-zinc-950 p-5">
            ...
            <div className="mb-4 flex flex-wrap items-center gap-3">
              {(
                ["draft", "active", "judging", "finished"] as ContestStatus[]
              ).map((s, i, arr) => {
                ...
                return (
                  <span key={s} className="flex items-center gap-1.5 text-xs">
                    <span
                      className={`border px-2 py-0.5 font-bold ${isCurrent ? meta.className : "border-white/10 text-zinc-600"}`}
                    >
                      {meta.label}
                    </span>
                    {i < arr.length - 1 && (
            ...
            {pendingTransition ? (
              <div className="border border-amber-400/40 bg-amber-400/[0.07] p-4">
                <p className="text-sm font-bold text-amber-200">
                  确认操作：{pendingTransition.label}
                </p>
                <p className="mt-1 text-sm leading-6 text-zinc-400">
                  {pendingTransition.impact}
                </p>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setPendingTransition({
                        ...
                        impact:
                          "开始后：允许参赛者投稿，题目按解锁计划开放。投稿通道将立即开启，请确认所有赛题时间和内容已就绪。",
                      })
                    }
                    disabled={saving}
                  ...
                  <button
                    ...
                    onClick={() =>
                      setPendingTransition({
                        ...
                        impact:
                          "进入评审后：正式投稿截止，参赛者仍可提交赛后补充；思路专区开放全文讨论和评分；榜单仍隐藏至结束。",
                      })
                    }
                    disabled={saving}
                  ...
                  <button
                    ...
                    onClick={() =>
                      setPendingTransition({
                        ...
                        impact:
                          "结束后：比赛彻底关闭，榜单和讨论区完全公开，思路专区继续开放（若设置了讨论时段则按时段限制）。此操作不可逆，请确认已完成奖项标注。",
                      })
                    }
                    disabled={saving}
        ...
        <section className="border border-white/10 bg-zinc-950 p-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <TextField
              ...
              onChange={(slug) => setContestForm({ ...contestForm, slug })}
            />
            <TextField
              label="标题"
              value={contestForm.title}
              onChange={(title) => setContestForm({ ...contestForm, title })}
            />
            ...
            <label className="grid gap-2 text-sm">
              ...
              <select
                ...
                onChange={(event) =>
                  setContestForm({
                    ...contestForm,
                    accessMode: event.target.value as ContestAccessMode,
                  })
                }
                className="h-11 border border-white/10 bg-black/20 px-3 text-white outline-none"
              >
              ...
              <span className="text-xs text-zinc-500">
                {accessModeMeta[contestForm.accessMode].description}
              </span>
            </label>
            <TextField
              label="简介"
              ...
              onChange={(description) =>
                setContestForm({ ...contestForm, description })
              }
            />
            <TextField
              label="开始时间"
              type="datetime-local"
              value={contestForm.startAt}
              onChange={(startAt) =>
                setContestForm({ ...contestForm, startAt })
              }
            />
            <TextField
              label="结束时间"
              type="datetime-local"
              value={contestForm.endAt}
              onChange={(endAt) => setContestForm({ ...contestForm, endAt })}
            />
            <TextField
              label="讨论开始时间"
              type="datetime-local"
              value={contestForm.discussionStartAt}
              onChange={(discussionStartAt) =>
                setContestForm({ ...contestForm, discussionStartAt })
              }
            />
            <TextField
              label="讨论结束时间"
              type="datetime-local"
              value={contestForm.discussionEndAt}
              onChange={(discussionEndAt) =>
                setContestForm({ ...contestForm, discussionEndAt })
              }
            />
            ...
            <div className="md:col-span-2">
              <TextArea
                label="一句话定位"
                value={contestForm.tagline}
                onChange={(tagline) =>
                  setContestForm({ ...contestForm, tagline })
                }
                rows={3}
              />
            </div>
            <div className="md:col-span-2">
              <TextArea
                ...
                onChange={(rules) => setContestForm({ ...contestForm, rules })}
                rows={6}
              />
            </div>
          </div>
          ...
          <>
            <section className="border border-white/10 bg-zinc-950 p-5">
                  ...
                  .map((item) => (
                    ...
                    >
                      <div className="flex flex-wrap items-start gap-3 p-4">
                        ...
                        <span
                          className={`mt-0.5 shrink-0 border px-2 py-0.5 text-xs font-bold ${contestProblemPhaseMeta[item.problem_phase ?? "daily"].className}`}
                        >
                          {
                            contestProblemPhaseMeta[
                              item.problem_phase ?? "daily"
                            ].label
                          }
                        </span>
                        <div className="min-w-0 flex-1">
                          ...
                          {item.problem_id ? (
                            ...
                            <p className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-zinc-400">
                              ...
                              {draftProblemMap.get(item.draft_problem_id)
                                ?.title ?? item.draft_problem_id}
                            </p>
                          ) : (
                          ...
                          <p className="mt-1 inline-flex items-center gap-1 text-xs text-zinc-500">
                            ...
                            {formatContestDateTime(item.open_at)} →{" "}
                            {formatContestDateTime(item.close_at)}（北京时间）
                          </p>
                        </div>
                      </div>
                      <div className="flex flex-wrap items-center gap-2 border-t border-white/[0.07] px-4 py-2.5">
                        <select
                          ...
                          onChange={(event) =>
                            updateContestProblem(item, {
                              unlock_mode: event.target
                                .value as DbContestProblem["unlock_mode"],
                            })
                          }
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            updateContestProblem(item, {
                              status: event.target
                                .value as DbContestProblem["status"],
                            })
                          }
                          className="h-8 border border-white/15 bg-zinc-950 px-2 text-xs text-zinc-300 outline-none"
                          title="状态"
                        >
                          {["locked", "open", "reviewing", "closed"].map(
                            (s) => (
                        ...
                        {(() => {
                          const isAutoTime =
                            (item.unlock_mode ?? "manual") === "auto_time";
                          const autoTimeHint =
                            "自动解锁模式下由开放/截止时间控制，此按钮已禁用。请先把「解锁方式」切换为「手动控制」再使用。";
                          return (
                            <>
                              <button
                                type="button"
                                onClick={() =>
                                  updateContestProblem(item, { status: "open" })
                                }
                                disabled={isAutoTime}
                        ...
                        {item.draft_problem_id &&
                          draftProblemMap.get(item.draft_problem_id)?.status !==
                            "promoted" && (
                            <button
                              type="button"
                              onClick={() =>
                                promoteDraft(item.draft_problem_id as string)
                              }
                              disabled={saving}
                      ...
                      <div className="flex flex-wrap items-center gap-2 border-t border-white/[0.07] px-4 py-2.5">
                        <select
                          ...
                          onChange={(event) =>
                            updateContestProblem(item, {
                              problem_phase: event.target
                                .value as ContestProblemPhase,
                            })
                          }
                          className="h-8 border border-white/15 bg-zinc-950 px-2 text-xs text-zinc-300 outline-none"
                          title="赛题阶段"
                        >
                          {Object.entries(contestProblemPhaseMeta).map(
                            ([value, meta]) => (
                              <option key={value} value={value}>
                                {meta.label}
                              </option>
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateContestProblem(item, {
                              score_max: Number(event.target.value),
                            })
                          }
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            updateContestProblem(item, {
                              score_policy: event.target
                                .value as ContestScorePolicy,
                            })
                          }
                          className="h-8 border border-white/15 bg-zinc-950 px-2 text-xs text-zinc-300 outline-none"
                          title="评分方式"
                        ...
                        <label className="inline-flex h-8 items-center gap-1.5 border border-white/15 px-2 text-xs text-zinc-300">
                          <input
                            ...
                            onChange={(event) =>
                              updateContestProblem(item, {
                                multiplier_eligible: event.target.checked,
                              })
                            }
                          />
                          吃挑战倍率
                        ...
                        <label className="inline-flex h-8 items-center gap-1.5 border border-white/15 px-2 text-xs text-zinc-300">
                          <input
                            ...
                            onChange={(event) =>
                              updateContestProblem(item, {
                                timed_mode_enabled: event.target.checked,
                              })
                            }
                          />
                          计时模式
                        </label>
                        {(item.problem_phase === "sprint" ||
                          item.timed_mode_enabled) && (
                          <>
                            <input
                              type="number"
                              ...
                              onChange={(event) =>
                                updateContestProblem(item, {
                                  answer_type: (event.target.value ||
                                    null) as ContestAnswerType | null,
                                })
                              }
                      ...
                      {(item.problem_phase === "sprint" ||
                        ...
                        <div className="border-t border-white/[0.07] px-4 py-2.5">
                          <AdminSprintAnswerKeyEditor
                            contestProblemId={item.id}
                            defaultAnswerType={item.answer_type}
                          />
                        </div>
                      )}
                    </div>
              ...
              <div className="mt-5 grid gap-4 border-t border-white/10 pt-5 md:grid-cols-2">
                <TextField
                  label="Day"
                  type="number"
                  value={String(problemForm.dayIndex)}
                  onChange={(dayIndex) =>
                    setProblemForm({
                      ...problemForm,
                ...
                <TextField
                  ...
                  onChange={(title) =>
                    setProblemForm({ ...problemForm, title })
                  }
                />
                <div className="grid gap-2 text-sm md:col-span-2">
                  ...
                  <div className="inline-flex w-fit border border-white/10">
                    <button
                      type="button"
                      onClick={() => {
                        setProblemSourceMode("public");
                        setProblemForm({ ...problemForm, draftProblemId: "" });
                      }}
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setProblemSourceMode("draft");
                        setProblemForm({ ...problemForm, problemId: "" });
                      }}
                  ...
                  <label className="grid gap-2 text-sm md:col-span-2">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setProblemForm({
                          ...problemForm,
                          draftProblemId: event.target.value,
                        })
                      }
                      className="h-11 border border-white/10 bg-black/20 px-3 text-sm text-white"
                    >
                      ...
                      {draftProblems
                        ...
                        .map((draft) => (
                          <option key={draft.id} value={draft.id}>
                            {draftSourceLabel(draft)} · {draft.title}
                          </option>
                        ))}
                    </select>
                    {draftProblems.filter(
                      (draft) => draft.status === "drafting",
                    ).length === 0 && (
                      <span className="text-xs text-zinc-600">
                        草稿箱还没有可选题目，请先到草稿箱页面新建。
                      </span>
                    )}
                  </label>
                )}
                <TextField
                  label="开放时间"
                  type="datetime-local"
                  value={problemForm.openAt}
                  onChange={(openAt) =>
                    setProblemForm({ ...problemForm, openAt })
                  }
                />
                <TextField
                  label="截止时间"
                  type="datetime-local"
                  value={problemForm.closeAt}
                  onChange={(closeAt) =>
                    setProblemForm({ ...problemForm, closeAt })
                  }
                />
                <TextField
                  label="权重"
                  type="number"
                  value={String(problemForm.weight)}
                  onChange={(weight) =>
                    setProblemForm({ ...problemForm, weight: Number(weight) })
                  }
                />
                <TextField
                  label="主题"
                  value={problemForm.theme}
                  onChange={(theme) =>
                    setProblemForm({ ...problemForm, theme })
                  }
                />
                <label className="grid gap-2 text-sm">
                ...
                <label className="grid gap-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setProblemForm({
                        ...problemForm,
                        problemPhase: event.target.value as ContestProblemPhase,
                      })
                    }
                    className="h-11 border border-white/10 bg-black/20 px-3 text-sm text-white"
                  >
                ...
                <TextField
                  label="满分"
                  type="number"
                  value={String(problemForm.scoreMax)}
                  onChange={(scoreMax) =>
                    setProblemForm({
                      ...problemForm,
                ...
                <label className="grid gap-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setProblemForm({
                        ...problemForm,
                        scorePolicy: event.target.value as ContestScorePolicy,
                      })
                    }
                    className="h-11 border border-white/10 bg-black/20 px-3 text-sm text-white"
                  >
                ...
                <label className="flex items-center gap-2 text-sm text-white">
                  <input
                    ...
                    onChange={(event) =>
                      setProblemForm({
                        ...problemForm,
                        multiplierEligible: event.target.checked,
                      })
                    }
                  />
                  吃挑战倍率（仅普通题应勾选）
                ...
                <label className="flex items-center gap-2 text-sm text-white">
                  <input
                    ...
                    onChange={(event) =>
                      setProblemForm({
                        ...problemForm,
                        timedModeEnabled: event.target.checked,
                      })
                    }
                  />
                  计时模式（计时题）
                </label>
                {(problemForm.problemPhase === "sprint" ||
                  problemForm.timedModeEnabled) && (
                  <>
                    <TextField
                      label="限时（秒）"
                      type="number"
                      value={problemForm.timeLimitSeconds}
                      onChange={(timeLimitSeconds) =>
                        setProblemForm({ ...problemForm, timeLimitSeconds })
                      }
                    />
                    <TextField
                      label="提交次数上限"
                      type="number"
                      value={String(problemForm.maxAttempts)}
                    ...
                    <label className="grid gap-2 text-sm">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setProblemForm({
                            ...problemForm,
                            answerType: event.target.value as
                              ContestAnswerType | "",
                          })
                        }
                        className="h-11 border border-white/10 bg-black/20 px-3 text-sm text-white"
                    ...
                    <TextField
                      label="答案格式说明（可选）"
                      value={problemForm.answerFormatNote}
                      onChange={(answerFormatNote) =>
                        setProblemForm({ ...problemForm, answerFormatNote })
                      }
                    />
                    <p className="text-xs leading-5 text-amber-400/80 md:col-span-2">
                      ⚠ 标准答案需要在数据库 contest_problem_answer_keys
                      表中单独配置，不会出现在此表单或任何公开查询里。
                    </p>
                  </>
                )}
            ...
            <section className="border border-white/10 bg-zinc-950 p-5">
              ...
              <div className="space-y-3">
                {(selectedContest.awards ?? []).map((award) => (
                  <div
                    key={award.id}
                    className="flex flex-col gap-3 border border-amber-400/20 bg-amber-400/[0.04] p-4 sm:flex-row sm:items-center sm:justify-between"
                  >
                    <div>
                      <p className="font-bold text-amber-100">{award.title}</p>
                      <p className="mt-1 text-xs text-zinc-500">
                        {contestAwardMeta[award.type]} · {award.points} 分
                      </p>
                      {(award.user_id ||
                        award.solution_id ||
                        award.submission_id) && (
                        <p className="mt-1 text-xs text-zinc-500">
                          {award.user_id &&
                            `获奖人：${awardParticipantMap.get(award.user_id)?.label ?? award.user_id.slice(0, 8)}`}
                          {award.user_id &&
                          (award.solution_id || award.submission_id)
                            ? " · "
                            : ""}
                          {award.solution_id &&
                            `正式解法：${awardSolutionMap.get(`solution:${award.solution_id}`)?.title ?? award.solution_id}`}
                          {award.submission_id &&
                            `比赛解法：${awardSolutionMap.get(`submission:${award.submission_id}`)?.title ?? award.submission_id}`}
                        </p>
                      )}
              ...
              <div className="mt-5 grid gap-4 border-t border-white/10 pt-5 md:grid-cols-2">
                <label className="grid gap-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setAwardForm({
                        ...awardForm,
                        type: event.target.value as ContestAwardType,
                      })
                    }
                    className="h-11 border border-white/10 bg-black/20 px-3 text-sm text-white"
                  >
                    {Object.entries(contestAwardMeta).map(([value, label]) => (
                      <option key={value} value={value}>
                        {label}
                      </option>
                    ))}
                ...
                <TextField
                  ...
                  onChange={(title) => setAwardForm({ ...awardForm, title })}
                />
                <label className="grid gap-2 text-sm">
                  <span className="font-bold text-white">关联题目</span>
                  <select
                    ...
                    onChange={(event) => {
                      ...
                      const selectedSolution = awardSolutions.find(
                        (solution) =>
                      ...
                      setAwardForm({
                        ...
                        problemId,
                        draftProblemId,
                        solutionId: selectedStillMatches
                          ? awardForm.solutionId
                          : "",
                        submissionId: selectedStillMatches
                  ...
                  >
                    ...
                    {contestAwardProblemOptions.map((problem) => (
                ...
                <TextField
                  label="加分"
                  type="number"
                  value={String(awardForm.points)}
                  onChange={(points) =>
                    setAwardForm({ ...awardForm, points: Number(points) })
                  }
                />
                <label className="grid gap-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setAwardForm({ ...awardForm, userId: event.target.value })
                    }
                    className="h-11 border border-white/10 bg-black/20 px-3 text-sm text-white"
                  >
                    ...
                    {awardParticipants.map((participant) => (
                      ...
                      >
                        {participant.label} · {participant.submissionCount}{" "}
                        次投稿
                      </option>
                    ))}
                  </select>
                ...
                <label className="grid gap-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateAwardSolution(event.target.value)
                    }
                    className="h-11 border border-white/10 bg-black/20 px-3 text-sm text-white"
                  >
                    ...
                    {filteredAwardSolutions.map((solution) => (
                      <option
                        key={awardSolutionValue(solution)}
                        value={awardSolutionValue(solution)}
                      >
                  ...
                  {!awardOptionsLoading &&
                    filteredAwardSolutions.length === 0 && (
                      <span className="text-xs text-zinc-600">
                        本场还没有可选解法；请先审核通过参赛解法，或只标获奖人。
                      </span>
                    )}
                </label>
                <div className="md:col-span-2">
            ...
            <section className="border border-white/10 bg-zinc-950 p-5">
              ...
              <p className="mb-5 text-xs text-zinc-500">
                ...
                {selectedContest.access_mode === "open" &&
                  " 下面只会列出被邀请或被暂停/移除等特殊状态的用户。"}
              </p>

              <div className="space-y-2">
                {registrationsLoading && (
                  <p className="text-xs text-zinc-600">正在加载报名记录...</p>
                )}
                ...
                {registrations.map((registration) => {
                  const meta =
                    contestRegistrationStatusMeta[registration.status];
                    ...
                    >
                      <div>
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-2">
                          <span
                            className={`inline-flex items-center gap-1 border px-2 py-0.5 text-[11px] font-bold ${meta.className}`}
                          >
                            {meta.label}
                          </span>
                          <span className="text-xs text-zinc-600">
                            {formatContestDateTime(registration.createdAt)}
                          </span>
                        </div>
                        {registration.note && (
                          <p className="mt-1 text-xs text-zinc-500">
                ...
                {inviteResults.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {inviteResults.map((result) => (
                      <div
                        key={result.id}
                        className="flex items-center justify-between gap-3 border border-white/[0.07] bg-black/20 px-3 py-2"
                      >
        ...
        <section className="border border-white/10 bg-black/20 p-4">
          <div className="flex items-center gap-2 text-xs text-zinc-500">
            ...
            如果保存失败，通常是线上数据库还没执行 contest
            migration，或当前账号不是 admin/moderator。
          </div>
        </section>
      </div>
    </div>

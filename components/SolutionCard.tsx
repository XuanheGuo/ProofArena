        ...
        <div className="bg-zinc-950 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {thinkingCues.observations.map((item) => (
              <span
                key={item}
                className="border border-white/10 bg-black/20 px-2.5 py-1 text-xs text-zinc-300"
              >
        ...
        <div className="bg-zinc-950 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {thinkingCues.keySignals.map((item) => (
              <span
                key={item}
                className="border border-amber-300/30 bg-amber-300/10 px-2.5 py-1 text-xs font-bold text-amber-100"
              >
                <MathBlock>{item}</MathBlock>
              </span>
            ))}
            ...
            <div className="md:min-w-56">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {thinkingCues.suggestedMethods.map((method) => (
                  <span
                    key={method}
                    className="border border-cyan-400/20 bg-cyan-400/5 px-2.5 py-1 text-xs text-cyan-100"
                  >
...
export function SolutionCard({
  ...
  const knowledgeNodes = (solution.knowledgeIds ?? [])
    ...
    .filter((node): node is NonNullable<ReturnType<typeof getKnowledgeNode>> =>
      Boolean(node),
    );
  const insightNodes = (solution.insightIds ?? [])
    .map(getInsightNode)
    .filter((node): node is NonNullable<ReturnType<typeof getInsightNode>> =>
      Boolean(node),
    );
  ...
  return (
    <article
      id={solution.id}
      className="scroll-mt-24 border border-white/10 bg-zinc-950"
    >
      <header className="grid border-b border-white/10 lg:grid-cols-[7rem_1fr_auto]">
        ...
        <div className="p-5 lg:p-7">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`border px-2 py-1 text-xs font-bold ${kindMeta.className}`}
            >
              {kindMeta.label}
            </span>
            ...
            {solution.tags.map((tag) => (
              <span
                key={tag}
                className="border border-white/10 px-2 py-1 text-xs text-zinc-400"
              >
                {tag}
              </span>
            ))}
          ...
          <p className="mt-2 text-sm text-zinc-500">
            {solution.author} <span className="mx-2 text-zinc-700">/</span>{" "}
            {solution.authorRole}
          </p>
          {solution.challenge && (
            <div className="mt-4  border border-amber-400/25 bg-amber-400/[0.06] px-3 py-2">
              <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-amber-200">
                ...
                <span>
                  挑战{" "}
                  {solution.challenge.targetSolutionTitle ??
                    solution.challenge.targetSolutionId}
                </span>
        ...
        <div className="flex items-center justify-between border-t border-white/10 px-5 py-4 lg:min-w-44 lg:flex-col lg:justify-center lg:border-t-0 lg:border-l">
          ...
          <span className="hidden text-center text-[11px] leading-5 text-zinc-600 lg:block">
            用于辅助比较，不代表唯一价值
          </span>
        </div>
      </header>
      ...
      <div
        className={view === "full" ? "grid lg:grid-cols-[1fr_21rem]" : "block"}
      >
        <div
          className={`p-5 md:p-7 ${view === "full" ? "lg:border-r lg:border-white/10" : ""}`}
        >
          <ThinkingCuesPanel solution={solution} />

          {view !== "idea" && (
            <section className="mt-6 border border-cyan-400/20 bg-cyan-400/[0.04]">
                  ...
                  <div className="bg-zinc-950 p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {solution.challenge.advantages.map((item) => (
                        <span
                          key={item}
                          className="border border-amber-400/25 bg-amber-400/5 px-2.5 py-1.5 text-xs text-amber-100"
                        >
                          {item}
                        </span>
                      ))}
          ...
          {view !== "idea" &&
            (knowledgeNodes.length > 0 || insightNodes.length > 0) && (
          ...
          {view !== "idea" &&
            solution.whyNotMethods &&
            solution.whyNotMethods.length > 0 && (
          ...
          <div
            className={`mt-6 grid gap-4 ${view !== "idea" ? "md:grid-cols-2" : ""}`}
          >
            ...
            <div className="mt-8">
              <div className="mb-5 flex items-center justify-between border-b border-white/10 pb-3">
                ...
                <span className="flex items-center gap-1.5 font-mono text-xs text-zinc-500">
                  <Clock3 className="size-3.5" />约 {solution.estimatedMinutes}{" "}
                  分钟
                </span>
              </div>
        ...
        {view === "full" && (
          <aside className="space-y-6 border-t border-white/10 p-5 md:p-7 lg:border-t-0">
            <div>
              <h3 className="mb-1 font-mono text-xs uppercase tracking-widest text-zinc-500">
                五维参考
              </h3>

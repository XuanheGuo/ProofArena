...
import {
  ...
  contestSolutionTypeOptions,
} from "@/lib/contest-meta";
import {
  ALLOWED_IMAGE_TYPES,
  MAX_CONTEST_THOUGHT_CHARS,
...
function getContestSubmissionState(
  ...
  const submitAccess = computeContestSubmitAccess(
    contest,
    registration,
    isLoggedIn,
  );
  ...
  if (
    ...
    return {
      ...
      label: "赛后补充",
      description:
        "比赛已结束，现在提交的内容会标记为赛后补充，不计入正式参赛提交。",
    };
  }
  ...
  if (contest.status === "active") {
    ...
    const effectiveStatus = getEffectiveProblemStatus(
      contestProblem,
      new Date(now),
    );
  ...
  return {
    ...
    label: "暂未开放提交",
    description:
      "当前比赛状态不允许提交，请等待管理员开放比赛或进入赛后补充阶段。",
  };
}
...
function computeSubmissionScopeKey(params: {
  ...
  if (params.contestProblemId)
    return `contest_problem:${params.contestProblemId}`;
  if (params.problemId)
    return params.isDraftProblem
      ? `draft_problem:${params.problemId}`
...
## 优势标签
${
  toLines(form.challengeAdvantages)
    .map((item) => `- ${item}`)
    .join("\n") || "（未填写）"
}
...
export function SubmitForm({
  ...
  const availableProblems = useMemo(
    () =>
      isContestMode
        ? problems.filter((problem) => contestProblemIds.has(problem.id))
        : problems,
    [contestProblemIds, isContestMode, problems],
  );
  const initialSelectedProblemId =
    initialProblemId &&
    availableProblems.some((problem) => problem.id === initialProblemId)
      ? initialProblemId
  ...
  const selectedProblem = useMemo(
    () =>
      availableProblems.find(
        (problem) => problem.id === solutionForm.problemId,
      ),
  ...
  const selectedChallengeSolution = useMemo(
    () =>
      selectedProblem?.solutions?.find(
        (solution) => solution.id === solutionForm.challengeTargetSolutionId,
      ),
  ...
  const activeScopeKey =
    mode === "solution"
      ? computeSubmissionScopeKey({
          ...
          isDraftProblem: Boolean(
            activeContestContext?.contestProblem?.draftProblemId,
          ),
        })
      : null;
  ...
  const cooldownUntilMs = rateLimitState
    ? new Date(rateLimitState.cooldownUntil).getTime()
    : null;
  const isCoolingDown = cooldownUntilMs !== null && cooldownUntilMs > now;
  const cooldownRemainingLabel = isCoolingDown
    ? formatCooldownRemaining(cooldownUntilMs! - now)
    : "";
  ...
  useEffect(() => {
    ...
    const stillExists = selectedProblem?.solutions?.some(
      (solution) => solution.id === solutionForm.challengeTargetSolutionId,
    );
    if (!stillExists) {
      setSolutionForm((current) => ({
  ...
  const restoreDraft = useCallback(() => {
    ...
    setSolutionForm((current) => ({
      ...
      contestSolutionType:
        saved.contestSolutionType ?? current.contestSolutionType,
  ...
  function handlePaste(event: React.ClipboardEvent) {
    ...
    if (pasted.length > 0) {
      setImageFiles((current) =>
        [...current, ...pasted].slice(0, MAX_IMAGE_COUNT),
      );
    }
  }
  ...
  async function submitVault() {
    ...
    const statement = vaultForm.statement
      .split(/\n+/)
      .map((l) => l.trim())
      .filter(Boolean);
    const { error: err } = await supabase.from("problem_drafts").insert({
      ...
      question_type:
        clampText(vaultForm.questionType, MAX_TITLE_CHARS) || "解答",
      tags,
  ...
  async function submitSolution(imageUrls: string[]) {
    if (activeContestContext && !contestSubmissionState.canSubmit) {
      return {
        data: null,
        error: { message: contestSubmissionState.description },
      };
    }

    const normalizedTitle =
      clampText(solutionForm.title, MAX_TITLE_CHARS) ||
      (activeContestContext?.contestProblem
    ...
    const normalizedForm = {
      ...
      approach: clampText(
        solutionForm.approach,
        activeContestContext
          ? MAX_CONTEST_THOUGHT_CHARS
          : MAX_GENERAL_TEXT_CHARS,
      ),
      keyTransform: clampText(
        solutionForm.keyTransform,
        MAX_GENERAL_TEXT_CHARS,
      ),
      steps: clampText(solutionForm.steps, MAX_GENERAL_TEXT_CHARS),
      insight: clampText(solutionForm.insight, MAX_GENERAL_TEXT_CHARS),
      verification: clampText(
        solutionForm.verification,
        MAX_GENERAL_TEXT_CHARS,
      ),
      challengeClaim: clampText(
        solutionForm.challengeClaim,
        MAX_GENERAL_TEXT_CHARS,
      ),
      challengeAdvantages: clampText(
        solutionForm.challengeAdvantages,
        MAX_GENERAL_TEXT_CHARS,
      ),
      challengeRisk: clampText(
        solutionForm.challengeRisk,
        MAX_GENERAL_TEXT_CHARS,
      ),
      observationWhy: clampText(
        solutionForm.observationWhy,
        MAX_GENERAL_TEXT_CHARS,
      ),
      transformationJustification: clampText(
        solutionForm.transformationJustification,
        MAX_GENERAL_TEXT_CHARS,
      ),
    };
    const markdown = buildSolutionMarkdown(
      normalizedForm,
      selectedProblem,
      activeContestContext,
      imageUrls,
    );
    const isPostContest = activeContestContext
      ? contestSubmissionState.isPostContest
      : false;
    ...
    return supabase
      ...
      .insert({
        ...
        contest_solution_type: activeContestContext
          ? solutionForm.contestSolutionType
          : null,
        ...
        content: {
          ...
          json: {
            solution: {
              ...
              transformationJustification:
                normalizedForm.transformationJustification,
              ...
              ...(initialForkSolutionId && !forkDismissed
                ? (() => {
                    const src = selectedProblem?.solutions?.find(
                      (s) => s.id === initialForkSolutionId,
                    );
                    return src
                      ? {
                          forkOf: {
                            solutionId: src.id,
                            solutionTitle: src.title,
                            solutionAuthor: src.author,
                          },
                        }
                      : {};
                  })()
  ...
  if (loading) {
    return <div className="h-56 animate-pulse bg-white/5" />;
  }
  ...
  if (!user) {
    return (
      <div className="space-y-4">
        <div
          className={`border p-4 ${isContestMode ? "border-amber-400/25 bg-amber-400/[0.06]" : "border-white/10 bg-black/20"}`}
        >
          <p
            className={`text-sm font-bold ${isContestMode ? "text-amber-100" : "text-white"}`}
          >
          ...
          {activeContestContext && (
            ...
            >
              <strong>{contestSubmissionState.label}</strong>：
              {contestSubmissionState.description}
            </p>
          )}
        </div>
        <div className="border border-amber-400/30 bg-amber-400/[0.06] p-6">
          <div className="flex items-start gap-3">
            <AlertCircle className="mt-0.5 size-5 shrink-0 text-amber-400" />
            <div>
              <p className="font-bold text-white">需要登录才能投稿</p>
              <p className="mt-2 text-sm leading-6 text-zinc-400">
                登录后可以提交新题或为已有题补充解法，所有内容都会先进入审核队列。
              </p>
              <Link
                href="/auth/login"
                className="mt-4 inline-flex h-10 items-center gap-2 bg-cyan-400 px-5 text-sm font-bold text-zinc-950 transition hover:bg-cyan-300"
              >
  ...
  return (
    <form onSubmit={handleSubmit} onPaste={handlePaste} className="space-y-5">
      ...
      >
        <div
          className={`size-2 shrink-0 ${isContestMode ? "bg-amber-400" : "bg-zinc-600"}`}
        />
        <div className="min-w-0 flex-1">
          <p
            className={`text-sm font-bold ${isContestMode ? "text-amber-200" : "text-white"}`}
          >
            {isContestMode ? "比赛投稿模式" : "普通投稿模式"}
          </p>
        ...
        <div className="flex items-start gap-3 border border-emerald-500/40 bg-emerald-500/[0.08] px-4 py-3">
          ...
          <div className="text-sm leading-6 text-zinc-300">
            {done === "problem" ? (
              ...
              <>
                <p>
                  已提交参赛思路，感谢参赛！管理员审核通过后，会进入本场比赛的思路专区，讨论阶段开放互评——不会自动进入正式题库。
                </p>
                {contestContext && (
                  <Link
        ...
        <div className="border border-amber-400/35 bg-amber-400/[0.07]">
          <div className="px-4 py-3">
            <p className="text-sm font-bold text-amber-200">
              {activeContestContext.contest.title}
            </p>
            ...
            <p
              className={`mt-2 border px-3 py-1.5 text-xs leading-5 ${
                contestSubmissionState.canSubmit
      ...
      {mode === "solution" ? (
        <div className="space-y-5">
          ...
          <label className="grid min-w-0 gap-2 text-sm">
            ...
            <select
              ...
              className="h-11 w-full min-w-0 border border-white/10 bg-zinc-900 px-3 text-sm text-white outline-none focus:border-cyan-400/50"
            >
          ...
          {isContestMode ? (
            <>
              <div className="grid gap-4 xl:grid-cols-2">
                <TextField
                  ...
                  onChange={(title) =>
                    setSolutionForm({ ...solutionForm, title })
                  }
                ...
                <label className="grid min-w-0 gap-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setSolutionForm({
                        ...solutionForm,
                        contestSolutionType: event.target
                          .value as ContestSolutionType,
                      })
                    }
                    className="h-11 w-full min-w-0 border border-white/10 bg-zinc-900 px-3 text-sm text-white outline-none focus:border-amber-400/50"
                  >
              ...
              <label className="flex items-start gap-3 border border-white/10 bg-black/20 px-4 py-3 text-sm">
                <input
                  ...
                  onChange={(event) =>
                    setSolutionForm({
                      ...solutionForm,
                      hideFromArena: event.target.checked,
                    })
                  }
              ...
              <TextArea
                ...
                onChange={(approach) =>
                  setSolutionForm({ ...solutionForm, approach })
                }
                rows={9}
            ...
            <>
                  ...
                  <section className="border border-amber-400/25 bg-amber-400/[0.055] p-4">
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      {selectedChallengeSolution && (
                        <span className="shrink-0 border border-amber-400/30 px-2.5 py-1 text-xs font-bold text-amber-200">
                          正在挑战：
                          <MathBlock>
                            {selectedChallengeSolution.title}
                          </MathBlock>
                        </span>
                      )}
                    </div>
                    <div className="mt-4 grid gap-4 xl:grid-cols-2">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setSolutionForm({
                              ...
                              challengeClaim: event.target.value
                                ? solutionForm.challengeClaim
                                : "",
                              challengeAdvantages: event.target.value
                                ? solutionForm.challengeAdvantages
                                : "",
                              challengeRisk: event.target.value
                                ? solutionForm.challengeRisk
                                : "",
                            })
                          }
                        ...
                        >
                          ...
                          {selectedProblem.solutions.map((solution) => (
                            <option key={solution.id} value={solution.id}>
                              {stripMathDelimiters(solution.title)} ·{" "}
                              {solution.author}
                            </option>
                          ))}
                        </select>
                      ...
                      <TextField
                        ...
                        onChange={(challengeClaim) =>
                          setSolutionForm({ ...solutionForm, challengeClaim })
                        }
                    ...
                    {solutionForm.challengeTargetSolutionId && (
                      <div className="mt-4 grid gap-4 xl:grid-cols-2">
                        ...
                        <TextArea
                          ...
                          onChange={(challengeRisk) =>
                            setSolutionForm({ ...solutionForm, challengeRisk })
                          }
                          rows={3}
                ...
                <label className="grid min-w-0 gap-2 text-sm">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setSolutionForm({
                        ...solutionForm,
                        kind: event.target.value as SolutionKind,
                      })
                    }
                    className="h-11 w-full min-w-0 border border-white/10 bg-zinc-900 px-3 text-sm text-white outline-none focus:border-cyan-400/50"
                  >
              ...
              <TextArea
                ...
                onChange={(keyTransform) =>
                  setSolutionForm({ ...solutionForm, keyTransform })
                }
                rows={3}
              ...
              <TextArea
                ...
                onChange={(steps) =>
                  setSolutionForm({ ...solutionForm, steps })
                }
                rows={8}
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                <TextArea
                  label="最值得学的地方"
                  value={solutionForm.insight}
                  onChange={(insight) =>
                    setSolutionForm({ ...solutionForm, insight })
                  }
                  rows={4}
                />
                <TextArea
                  label="可验证位置"
                  value={solutionForm.verification}
                  onChange={(verification) =>
                    setSolutionForm({ ...solutionForm, verification })
                  }
                  rows={4}
                />
              </div>
              <CASVerifier
                steps={solutionForm.steps.split("\n").filter(Boolean)}
              />
              <details className="group border border-white/10 bg-black/20">
                ...
                <div className="space-y-3 border-t border-white/10 px-4 pb-4 pt-3">
                  <TextArea
                    ...
                    onChange={(observationWhy) =>
                      setSolutionForm({ ...solutionForm, observationWhy })
                    }
                    rows={2}
                  ...
                  <TextArea
                    ...
                    onChange={(transformationJustification) =>
                      setSolutionForm({
                        ...solutionForm,
                        transformationJustification,
                      })
        ...
        <div className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-3">
            <TextField
              label="年份/赛季"
              value={vaultForm.year}
              onChange={(year) => setVaultForm({ ...vaultForm, year })}
              placeholder="2026"
            />
            <SelectField
              required
            ...
            <TextField
              label="来源说明"
              value={vaultForm.paper}
              onChange={(paper) => setVaultForm({ ...vaultForm, paper })}
              placeholder="原创命题 / 改编自某题 / 灵感来源"
            />
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            <TextField
              label="编号/槽位"
              value={vaultForm.number}
              onChange={(number) => setVaultForm({ ...vaultForm, number })}
              placeholder="Weekly01 D1-A / 第 1 题"
            />
            <SelectField
              label="难度"
              value={vaultForm.difficulty}
              onChange={(difficulty) =>
                setVaultForm({ ...vaultForm, difficulty })
              }
            ...
            <SelectField
              ...
              onChange={(questionType) =>
                setVaultForm({ ...vaultForm, questionType })
              }
          ...
          <TextField
            ...
            onChange={(title) => setVaultForm({ ...vaultForm, title })}
            placeholder="用一句话概括题目主题，可含 LaTeX，如：关于 $x^2$ 的不等式"
          />
          <MathPreviewTextArea
            required
            label="完整题干"
            value={vaultForm.statement}
            onChange={(statement) => setVaultForm({ ...vaultForm, statement })}
            rows={8}
            placeholder="在此输入题目，$x^2 + y^2 = r^2$，支持行内公式 $...$ 和块公式 $$...$$"
          />
          <MathPreviewTextArea
            label="标准答案"
            value={vaultForm.answer}
            onChange={(answer) => setVaultForm({ ...vaultForm, answer })}
            rows={3}
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <TextArea
              ...
              onChange={(tags) => setVaultForm({ ...vaultForm, tags })}
              rows={3}
            ...
            <TextArea
              ...
              onChange={(notes) => setVaultForm({ ...vaultForm, notes })}
              rows={3}
              placeholder="命题意图、改编幅度、审核提示；没有严格出处也可以留空"
            />
          </div>
        </div>
      ) : (
        <div className="space-y-5">
          <div className="grid gap-4 xl:grid-cols-2">
            <TextField
              ...
              onChange={(source) => setProblemForm({ ...problemForm, source })}
            ...
            <TextField
              ...
              onChange={(title) => setProblemForm({ ...problemForm, title })}
          ...
          <MathPreviewTextArea
            ...
            onChange={(statement) =>
              setProblemForm({ ...problemForm, statement })
            }
            rows={8}
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <MathPreviewTextArea
              ...
              onChange={(answer) => setProblemForm({ ...problemForm, answer })}
              rows={4}
              placeholder="答案，支持 LaTeX"
            />
            <TextArea
              ...
              onChange={(tags) => setProblemForm({ ...problemForm, tags })}
              rows={4}
              placeholder="导数、圆锥曲线、数列"
            />
          </div>
          <TextArea
            ...
            onChange={(note) => setProblemForm({ ...problemForm, note })}
            rows={3}
      ...
      {precheckFailedReason !== null && (
        <div className="border border-amber-400/30 bg-amber-400/[0.06] px-4 py-3">
          <p className="text-sm text-amber-300">
            预筛未通过：{getSubmissionFailureReasonLabel(precheckFailedReason)}
            ，请修改后重新提交。
          </p>
        </div>
      )}
      ...
      {error && (
        <div className="border border-red-400/30 bg-red-400/[0.06] px-4 py-3">
          <p className="text-sm text-red-300">{error}</p>
        </div>
      )}

      {isCoolingDown && (
        <div className="border border-orange-400/30 bg-orange-400/[0.06] px-4 py-3">
          <p className="text-sm text-orange-300">
            提交冷却中，请在{cooldownRemainingLabel}后重试。
          </p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-3 border-t border-white/10 pt-5 sm:flex-row sm:items-center sm:justify-between">
        <button
          ...
          disabled={
            submitting ||
            isCoolingDown ||
            (mode === "solution" && availableProblems.length === 0) ||
            (mode === "solution" &&
              activeContestContext &&
              !contestSubmissionState.canSubmit)
          }
          className="inline-flex h-11 items-center justify-center gap-2 bg-cyan-400 px-8 text-sm font-bold text-zinc-950 transition hover:bg-cyan-300 disabled:opacity-50"
        >
  ...
  return (
    <label className="grid min-w-0 gap-2 text-sm">
      ...
      <input
        ...
        className="h-11 w-full min-w-0 border border-white/10 bg-white/5 px-4 text-sm text-white outline-none transition placeholder:text-zinc-600 focus:border-cyan-400/50"
      />
    </label>
  );
}
  ...
  return (
    <label className="grid min-w-0 gap-2 text-sm">
      ...
      <select
        ...
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="h-11 w-full min-w-0 border border-white/10 bg-white/5 px-4 text-sm text-white outline-none transition focus:border-cyan-400/50"
      >
        {options.map((option) => (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-zinc-600">
              最多 {MAX_IMAGE_COUNT} 张，支持{" "}
              {ALLOWED_IMAGE_TYPES.map((type) =>
                type.replace("image/", "").toUpperCase(),
              ).join(" /")}
              ，单张不超过 {Math.round(MAX_IMAGE_BYTES / 1024 / 1024)}
              MB。支持粘贴截图（Ctrl/Cmd+V）。
            </p>
          </div>
        </div>
  ...
  return (
    <label className="grid min-w-0 gap-2 text-sm">
      ...
      <textarea
        ...
        onChange={(event) => onChange(event.target.value)}
        placeholder={placeholder}
        className="w-full min-w-0 resize-y border border-white/10 bg-white/5 px-4 py-3 text-sm leading-6 text-white outline-none transition placeholder:text-zinc-600 focus:border-cyan-400/50"
      />
    </label>
  );
}

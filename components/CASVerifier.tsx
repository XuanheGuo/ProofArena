...
import { CheckCircle2, XCircle, AlertCircle, Loader2, FlaskConical, Minus } from 'lucide-react';
...
function StepRow({ v }: { v: StepVerification }) {
  const skipped = v.method === 'no_math' || v.method === 'no_equation' || v.method === 'condition';
  ...
  return (
    <div className={`flex gap-2 p-2 rounded border text-xs ${bg}`}>
      {icon}
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5 mb-0.5">
          <span className="text-[11px] text-gray-400">步骤 {v.index + 1}</span>
          <span className="text-[10px] text-gray-600 px-1 rounded bg-black/20">
            {METHOD_LABEL[v.method] ?? v.method}
          </span>
        </div>
        <div className="text-gray-300 break-words">{v.step}</div>
        {v.detail && (
          <div className={`mt-0.5 text-[11px] ${v.valid === false ? 'text-red-400' : 'text-gray-500'}`}>
            {v.detail}
          </div>
        )}
        {v.error && (
...
export function CASVerifier({ steps, answerA, answerB }: CASVerifierProps) {
  ...
  const [equivResult, setEquivResult] = useState<{ equivalent: boolean | null; method: string; error?: string } | null>(null);
  ...
  async function runEquivCheck() {
    ...
    try {
      ...
      setEquivResult({ equivalent: null, method: 'parse_error', error: '连接 CAS 服务失败' });
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {stepsStatus === 'loading' ? (
            ...
            <span className="text-xs text-gray-400">{validSteps.length} 步</span>
          </div>
          {stepsStatus !== 'idle' && (
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md bg-teal-600 text-white hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {equivStatus === 'loading' ? (
                <Loader2 className="w-3 h-3 animate-spin" />
              ) : (
            ...
            <div className={`flex gap-2 items-start p-2 rounded border text-xs ${
              ...
              <div>
                <span className="font-medium text-gray-200">
                  {equivResult.equivalent === true ? '等价' : equivResult.equivalent === false ? '不等价' : '无法判断'}
                </span>
                <span className="ml-1.5 text-gray-500 text-[11px]">({METHOD_LABEL[equivResult.method] ?? equivResult.method})</span>
                {equivResult.error && (

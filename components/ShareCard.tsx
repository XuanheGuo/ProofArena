...
export function getShareRoutes(problem: Problem) {
  ...
  const robust =
    [...problem.solutions].sort((a, b) => {
      ...
      return (
        b.scores.correctness +
        b.scores.examReady +
        bRobust -
        (a.scores.correctness + a.scores.examReady + aRobust)
      );
    })[0] ?? exam;
...
export function getShareTags(problem: Problem) {
  ...
  return [
    ...new Set([...knowledgeTags, ...insightTags, ...problem.tags]),
  ].slice(0, 5);
}
...
export function getDefaultShareSolution(problem: Problem) {
  return (
    [...problem.solutions].sort((a, b) => {
      ...
      return (
        bSignal +
        b.scores.explanation +
        b.scores.elegance -
        (aSignal + a.scores.explanation + a.scores.elegance)
      );
...
const shareModeCopy: Record<
  ShareCardMode,
  { eyebrow: string; title: string; minHeight: string }
> = {
  idea: {
    eyebrow: "思路卡",
    title: "这一步是怎么想到的",
    minHeight: "min-h-[600px] xl:min-h-[460px]",
  },
  transform: {
    eyebrow: "转化卡",
    title: "关键转化",
  ...
  full: {
    eyebrow: "完整过程",
    title: "从观察到落笔",
    minHeight: "min-h-[800px] xl:min-h-[580px]",
  },
};
...
function ShareSection({
  ...
  return (
    <div className="border-l-2 border-cyan-400/75 py-0.5 pl-3">
      ...
      <div className="mt-1.5 break-words text-xs leading-5 text-zinc-100">
        {children}
      </div>
    </div>
  );
...
export function ShareCard({
  ...
  const tradeoff =
    [...solution.tradeoffs, ...solution.limitations][0] ??
    "需要结合题目条件判断适用范围。";
        ...
        <div className="mt-5 xl:grid xl:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] xl:gap-4">
          <div>
            <div className="flex flex-wrap gap-2 text-[11px] font-bold">
              <span className="bg-cyan-400 px-2 py-1 text-zinc-950">
                {problem.region}
              </span>
            ...
            <p className="mt-3 text-xs font-bold uppercase tracking-widest text-zinc-500">
              {modeCopy.eyebrow} · 本题解法
            </p>
            <h2 className="mt-1.5 text-[1.7rem] font-black leading-tight text-white sm:text-4xl">
              {solution.title}
            </h2>
            ...
            <div className="mt-5 flex items-end justify-between gap-4 border-y border-white/10 py-3">
              <div className="min-w-0">
                ...
                <strong
                  className={`mt-1.5 inline-flex border px-2 py-1 text-xs ${kindMeta.className}`}
                >
                  {kindMeta.label}
                </strong>
              ...
              <div className="shrink-0 border-l border-white/10 pl-4 text-right">
                ...
                <strong className="mt-1 block font-display text-2xl text-cyan-300">
                  {(
                    (solution.scores.correctness +
                      solution.scores.examReady +
            ...
            <div className="mt-5 border-t border-white/10 pt-4">
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="border border-cyan-400/20 bg-cyan-400/5 px-2.5 py-1.5 text-[11px] font-bold text-cyan-100"
                  >
                    #{tag}
                  </span>
                ))}
